...
**Theorem 2.11** - If \(A\) and \(B\) are independent, then \(A\) and \(B^\complement\) are also independent.
...
**Independent of more than two events** - Events \(A_1, A_2, \ldots, A_k\) are **independent** if and only if the probability of the intersections of any 2, 3,..., or \(k\) of these events equals the product of their respective probabilities.



## 2.8 Bayes Theorem

**Theorem 2.12 (aka Rule of Total probability or rule of elimination)** - If the events \(B_1, B_2, \ldots, B_k\) constitue a partition of the sample space \(S\) and \(P(B_i) \ne 0\) for \(i=1,2,\ldots,k\) then for any event \(A\) in \(S\)
...
**Proof** Writing \(P(B_r|A) = \frac{P(A \cap B_r)}{P(A)}\) in accordance with the definition of conditional probability, we have only to substitute \(P(B_r) \cdot P(A|B_r)\) for \(P(A \cap B_r)\) and the formul of Theorem 2.12 for \(P(A)\)
