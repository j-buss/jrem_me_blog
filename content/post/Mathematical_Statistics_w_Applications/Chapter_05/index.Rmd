...
**Theorem 5.4 [Binomial Moment Generation]** - The moment-generating fucntion of the binomial distribution is given by 
...
where we omitted the term corrsponding to \(x=0\), which is 0, and canceled the \(x\) against the first factor of \(x!=x(x-1)!\) in the denominator of \(\dbinom M x\). Then factoring out \(M/\dbinom N n\), we get
...
To obtain the formula for \(\sigma^2\), we proceed as in the proof of Theorem 5.2 [Binomial Distribution - Mean and Variance] by first evaluating \(E[X(x-1]\) and then making use of the fact that \(E(X^2)=E[X(X-1)]+E(X)\). We thus get
...
Using the definition for Moment-generating Functions
...
where \(p\) is the actual proportion of defectives in teh log
